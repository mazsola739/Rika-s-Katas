...
in a packed stadium when successive groups of spectators briefly stand, yell, and raise their arms. Immediately upon stretching to full 
...
function wave(str) {
  let arr = (letter, index, array) => {
    let character = array.slice();
    character[index] = character[index].toUpperCase();
    return character.join("");
  };
  return str
    .split("")
    .map(arr)
    .filter((space, i) => space[i] !== " ");
}
